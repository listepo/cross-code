 ...
 * specs in `app/tests/`.
 *
 * Both run on the device's own WAMR interpreter: the demo page renders the
 ...
 */
import type { WamrImports, WamrModule, WamrRuntime, WasmArg, WasmValue } from '@org/nativescript-wamr';
 ...
 */
export function createHostImports(log: HostCall[]): WamrImports {
  const sink =
    (fn: string) =>
 ...
 */
export function runFixtureChecks(module: WamrModule, log: HostCall[]): Check[] {
 ...
 */
export function runGlobalsChecks(module: WamrModule): Check[] {
  const checks: Check[] = [];
