...
Both go through the plugin's public TypeScript API onto the device's own wasm3
...
### 4. The specs run against real wasm3, so error text comes from wasm3

Assertions on failure messages match wasm3's own strings — `function lookup
failed`, `missing imported function` (`src/vendors/wasm3/wasm3.h`) — with the
offending name appended by wasm3 as a quoted detail. Do not assert on the exact
...
wasm3 compiles lazily, so a missing import surfaces at `findFunction` — not at
`loadModule` and not at call time. `fixture-module.spec.ts` asserts exactly that.
...
The plugin is consumed as built `dist/`, so `nx run nativescript-wasm3:build`
