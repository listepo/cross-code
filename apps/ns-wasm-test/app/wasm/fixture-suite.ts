...
type FixtureArgs<K extends FixtureFunction> = Parameters<
  Extract<FixtureApi[K], AnyFn>
>;
type FixtureResult<K extends FixtureFunction> = ReturnType<
  Extract<FixtureApi[K], AnyFn>
>;
...
export function callFixture<K extends FixtureFunction>(
  ...
  return module.call(
    name,
    ...(args as readonly unknown[] as WasmArg[]),
  ) as FixtureResult<K>;
}
...
export function runFixtureChecks(
  ...
  checks.push(
    check(
      'add_i32 wraps at i32::MAX',
      call('add_i32', 2147483647, 1),
      -2147483648,
    ),
  );
  ...
  checks.push(
    check(
      'add_i64 past 2^53',
      call('add_i64', 9007199254740993n, 2n),
      9007199254740995n,
    ),
  );
  checks.push(
    check(
      'mul_i64(2^31, 2^31)',
      call('mul_i64', 2147483648n, 2147483648n),
      4611686018427387904n,
    ),
  );
  checks.push(
    check('neg_i64(2^40)', call('neg_i64', 1099511627776n), -1099511627776n),
  );
  checks.push(check('i64_max()', call('i64_max'), 9223372036854775807n));
  checks.push(check('i64_min()', call('i64_min'), -9223372036854775808n));
  checks.push(
    check(
      'identity_i64 keeps every bit',
      call('identity_i64', -9007199254740993n),
      -9007199254740993n,
    ),
  );
  ...
  checks.push(
    check(
      'mixed_args(1, 2n, 0.5, 0.25)',
      call('mixed_args', 1, 2n, 0.5, 0.25),
      3.75,
    ),
  );
  ...
  checks.push(
    check('counter_i32_inc accumulates', call('counter_i32_inc', -2), 3),
  );
  checks.push(check('counter_i32_get after inc', call('counter_i32_get'), 3));
  call('counter_i64_reset');
  checks.push(
    check(
      'counter_i64_inc(2^32)',
      call('counter_i64_inc', 4294967296n),
      4294967296n,
    ),
  );
  ...
  checks.push(
    check('accum_f64_add accumulates', call('accum_f64_add', 2.25), 3.75),
  );
  ...
  checks.push(
    check('call_log_i64 kept 2^40 as bigint', logged[1]?.value, 1099511627776n),
  );
  ...
  checks.push(
    check('call_transform_i32(21)', call('call_transform_i32', 21), 42),
  );
  checks.push(
    check(
      'call_transform_i64(2^40)',
      call('call_transform_i64', 1099511627776n),
      2199023255552n,
    ),
  );
  checks.push(
    check('call_transform_f32(1.25)', call('call_transform_f32', 1.25), 2.5),
  );
  checks.push(
    check('call_transform_f64(1.25)', call('call_transform_f64', 1.25), 2.5),
  );
  ...
  checks.push(
    checkThat(
      'mem_scratch_ptr() is inside memory',
      scratch > 0 && scratch < module.runtime.memorySize,
      scratch,
      `0 < ptr < ${module.runtime.memorySize}`,
    ),
  );

  call('mem_write_u8', scratch, 0xab);
  checks.push(
    check('wasm write → wasm read', call('mem_read_u8', scratch), 0xab),
  );
  checks.push(
    check(
      'wasm write → host read',
      module.runtime.readMemory(scratch, 1)[0],
      0xab,
    ),
  );
  ...
  checks.push(
    check(
      'host write → wasm read (little endian)',
      call('mem_read_i32', scratch + 4),
      -272716322,
    ),
  );
  checks.push(
    check(
      ...
      [...module.runtime.readMemory(scratch + 4, 4)].join(),
      '222,173,190,239',
    ),
  );
...
export function runGlobalsChecks(module: WasmModuleLike): Check[] {
  ...
  checks.push(
    check('g_i64 initial (2^32)', module.getGlobal('g_i64'), 4294967296n),
  );
  ...
  checks.push(
    check(
      'g_i64 after setGlobal past 2^53',
      module.getGlobal('g_i64'),
      9007199254740993n,
    ),
  );
