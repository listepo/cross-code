...
export interface RstestResultsViewOptions {
...
export class RstestResultsView extends GridLayout {
  ...
  constructor(options: RstestResultsViewOptions = {}) {
    super();
  ...
  private render(snap: NativeScriptResultSnapshot): void {
    ...
    this.titleLabel.text = ` rstest ${snap.status === 'running' ? '↻' : snap.status === 'passed' ? '✓' : '✗'} `;
  ...
  private renderTree(node: TreeNode, parent: StackLayout, depth: number, prefix: string[]): void {
    ...
    children.forEach((child, idx) => {
      const isLast = idx === children.length - 1 && testCount === 0;
      const branch = isLast ? '└─ ' : '├─ ';
...
export function createRstestResultsPage(
  source: NativeScriptTestEventSource,
): Page {
  const page = new Page();
  page.actionBarHidden = true;
  page.content = new RstestResultsView({ source });
  return page;
}
