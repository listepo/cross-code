import {
  describeTestCase,
  describeTestResult,
  type NativeScriptTestDescriptor,
...
/**
 * Folds device run events into a renderable snapshot. Shared by the on-device
 * results view and any host-side consumer of the same event stream.
 */
  ...
  apply(event: NativeScriptTestEvent): void {
    switch (event.type) {
      case 'run-started':
        ...
        break;
      case 'file-start':
        this.files.add(event.testPath);
        break;
      case 'case-start': {
        const test = describeTestCase(event.test);
        this.tests.set(test.id, test);
        this.files.add(test.file);
        break;
      }
      case 'case-result': {
        const test = describeTestResult(event.result);
        this.tests.set(test.id, test);
        this.files.add(test.file);
        break;
      }
      case 'file-result':
        this.files.add(event.result.testPath);
        break;
      case 'run-finished':
