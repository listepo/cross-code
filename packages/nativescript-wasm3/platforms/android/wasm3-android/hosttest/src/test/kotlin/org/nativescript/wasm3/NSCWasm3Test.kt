...
class NSCWasm3Test {
    ...
    @Test
    fun version() {
        assertEquals("0.5.2", NSCWasm3Runtime.wasm3Version())
    }
    ...
    fun unlinkedImportFails() = withSuite { runtime, _ ->
        ...
        val error = assertFailsWith<NSCWasm3Exception> {
            runtime.findFunction("call_host_add")
        }
        assertTrue(error.message!!.contains("missing imported function"))
    }
    ...
    fun memoryAccess() = withSuite { runtime, _ ->
        assertEquals(64 * 1024, runtime.memorySize())
        ...
        assertFailsWith<NSCWasm3Exception> { runtime.readMemory(64 * 1024 - 2, 4) }
        assertFailsWith<NSCWasm3Exception> { runtime.writeMemory(64 * 1024, byteArrayOf(1)) }
    }
    ...
    fun globals() = withSuite { runtime, module ->
        ...
        assertFailsWith<NSCWasm3Exception> { module.getGlobal("nope") }
    }

    @Test
    fun argumentValidation() = withSuite { runtime, _ ->
        val add = runtime.findFunction("add_i32")
        assertFailsWith<NSCWasm3Exception> { add.call(arrayOf(1)) }
    ...
    fun invalidModuleBytes() {
        NSCWasm3Runtime().use { runtime ->
            assertFailsWith<NSCWasm3Exception> {
