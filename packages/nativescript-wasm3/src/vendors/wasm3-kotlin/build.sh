#!/bin/bash
# Build the Rust FFI library and generate Kotlin bindings for wasm3-kotlin.
#
...
build_rust() {
    ...
    cp target/release/libwasm3_ffi.dylib "$host_dir/"
    echo "  -> Copied libwasm3_ffi.dylib to $host_dir/"
}
...
build_android() {
    ...
    for abi_pair in "${abis[@]}"; do
        ...
        cargo build -p wasm3-ffi --lib --release --target "$target" 2>&1 || {
            echo "  WARNING: Failed to build for $target — skipping"
            continue
        }

        mkdir -p "$jni_dir"
        cp "target/$target/release/libwasm3_ffi.so" "$jni_dir/libwasm3_ffi.so"
        echo "  -> Copied libwasm3_ffi.so to $jni_dir/"
    done
}
