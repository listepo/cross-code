...
- **Android** — Kotlin library. JNI bindings for the wasm3 C API are
  auto-generated by [JavaCPP](https://github.com/bytedeco/javacpp) from
  `wasm3.h`; a Kotlin wrapper (`org.nativescript.wasm3.*`) provides the
  runtime API. Ships as a prebuilt `.aar` with `libjniwasm3.so` for
...
platforms/
  ...
  android/
    ...
    nativescript-wasm3.aar prebuilt library (Kotlin + generated JNI + .so files)
...
(`platforms/android/wasm3-android/build-native.mjs`, a zx script Gradle shells
...
**Android: `UnsatisfiedLinkError` for `libjniwasm3`** — the native `.so` isn't
