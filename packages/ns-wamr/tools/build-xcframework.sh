#!/usr/bin/env bash
# Builds NSCWamr.xcframework (device arm64 + simulator arm64 + simulator
...
#   platforms/ios/NSCWamr.xcframework        — shipped to the app
#   platforms/ios/NSCWamr.xcframework.dSYMs/ — kept debug symbols (device+sim)
set -euo pipefail
cd "$(dirname "$0")/../platforms/ios/NSCWamr"

OUT=../NSCWamr.xcframework
DSYM_OUT=../NSCWamr.xcframework.dSYMs
...
build_slice() { # $1=triple $2=sdk-name $3=output-name
  ...
  swift build --disable-sandbox -c release --product NSCWamr --triple "$triple" \
    -Xswiftc -sdk -Xswiftc "$sdk" \
    -Xcc -isysroot -Xcc "$sdk" \
    -Xswiftc -g -Xswiftc -Osize \
    -Xcc -g -Xcc -flto=thin \
    -Xlinker -dead_strip
  local dylib=".build/$triple/release/libNSCWamr.dylib"

  # Assemble the framework bundle around the dylib.
  local fw="$BUILD/$out_name/NSCWamr.framework"
  mkdir -p "$fw/Headers" "$fw/Modules"
  cp "$dylib" "$fw/NSCWamr"
  cp Sources/NSCWamr/include/*.h "$fw/Headers/"
  ...
  # only and works).
  cp Sources/NSCWamr/include/module.modulemap "$fw/Modules/"
  ...
  install_name_tool -id "@rpath/NSCWamr.framework/NSCWamr" "$fw/NSCWamr"
  /usr/libexec/PlistBuddy -c "Clear dict" \
    -c "Add :CFBundleIdentifier string org.nativescript.NSCWamr" \
    -c "Add :CFBundleName string NSCWamr" \
    -c "Add :CFBundleExecutable string NSCWamr" \
  ...
  dsymutil "$fw/NSCWamr" -o "$BUILD/$out_name/NSCWamr.framework.dSYM" >/dev/null 2>&1 || true
}
...
strip -S -x "$BUILD/ios-arm64/NSCWamr.framework/NSCWamr"
strip -S -x "$BUILD/ios-arm64-simulator/NSCWamr.framework/NSCWamr"
strip -S -x "$BUILD/ios-x86_64-simulator/NSCWamr.framework/NSCWamr"
...
<dict>
	...
	<array>
		<dict>
			...
			<key>LibraryPath</key>
			<string>NSCWamr.framework</string>
...
PLIST
ditto "$BUILD/ios-arm64/NSCWamr.framework" "$OUT/ios-arm64/NSCWamr.framework"
ditto "$BUILD/ios-arm64-simulator/NSCWamr.framework" "$OUT/ios-arm64-simulator/NSCWamr.framework"
ditto "$BUILD/ios-x86_64-simulator/NSCWamr.framework" "$OUT/ios-x86_64-simulator/NSCWamr.framework"

# ── 5. Keep the dSYMs produced by dsymutil (device + simulator). ─────────
for slice in ios-arm64 ios-arm64-simulator ios-x86_64-simulator; do
  ditto "$BUILD/$slice/NSCWamr.framework.dSYM" "$DSYM_OUT/$slice/NSCWamr.framework.dSYM"
done
