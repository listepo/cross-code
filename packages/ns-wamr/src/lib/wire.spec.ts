...
describe('parseSignature', () => {
  ...
  it('rejects malformed signatures', () => {
    for (const bad of ['', 'x(i)', 'i(q)', 'i(i', 'ii', '(i)i']) {
      expect(() => parseSignature(bad), bad).toThrow(WamrError);
    }
  });
});
...
describe('toWire / fromWire', () => {
  ...
  it('rejects non-wasm values', () => {
    expect(() => toWire({} as never, 'arg')).toThrow(WamrError);
    expect(() => toWire(undefined as never, 'arg')).toThrow(WamrError);
  });
  ...
  it('rejects bad i64 wire values', () => {
    expect(() => fromWire('i64', 'not-a-number')).toThrow(WamrError);
  });
});
...
describe('hostResultToWire', () => {
  ...
  it('rejects result count mismatches', () => {
    expect(() => hostResultToWire(['i32'], undefined, 'ctx')).toThrow(WamrError);
    expect(() => hostResultToWire([], 1, 'ctx')).toThrow(WamrError);
    expect(() => hostResultToWire(['i32'], [1, 2], 'ctx')).toThrow(WamrError);
  });
});
