...
interface NSCWamrRuntimeRef {
  readonly memorySize: number;
  loadModuleError(bytes: Uint8Array, ...error: NativeErrorRef[]): NSCWamrModuleRef | null;
  loadModuleFromFileError(path: string, ...error: NativeErrorRef[]): NSCWamrModuleRef | null;
  findFunctionError(name: string, ...error: NativeErrorRef[]): NSCWamrFunctionRef | null;
...
interface NSCWamrRuntimeClass {
  alloc(): {
    initWithStackSizeWasiEnabledExecutionTier(
      stackSizeInBytes: number,
      wasiEnabled: boolean,
      // The Swift initializer exposes the tier as a String to ObjC.
 ...
 */
interface NSCWamrHostCallbackClass {
...
declare var NSCWamrRuntime: NSCWamrRuntimeClass | undefined;
declare var NSCWamrHostCallback: NSCWamrHostCallbackClass | undefined;

// ---------------------------------------------------------------------------
// Android — Kotlin classes packaged in nativescript-wamr.aar
// ---------------------------------------------------------------------------

interface NSCWamrJavaFunction {
  getName(): string;
...
interface NSCWamrJavaRuntime {
  loadModule(bytes: unknown): NSCWamrJavaModule;
  loadModuleFromFile(path: string): NSCWamrJavaModule;
  findFunction(name: string): NSCWamrJavaFunction;
  memorySize(): number;
...
interface NSCWamrJavaRuntimeClass {
  new (stackSizeInBytes: number, wasiEnabled: number, executionTier: number): NSCWamrJavaRuntime;
  wamrVersion(): string;
}

interface NSCWamrHostFunctionClass {
...
interface NSCWamrAndroidNamespace {
  NSCWamrRuntime: NSCWamrJavaRuntimeClass;
  NSCWamrHostFunction: NSCWamrHostFunctionClass;
}

declare var org:
  | { nativescript?: { wamr?: NSCWamrAndroidNamespace } }
  | undefined;
