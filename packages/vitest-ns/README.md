...
- The optional `@cross-code/vitest-ns-ui` package listens to the same
...
```bash
# NativeScript app
pnpm add @cross-code/vitest-ns @nativescript/core @valor/nativescript-websockets

# Vitest workspace
pnpm add -D @cross-code/vitest-ns vitest @vitest/runner @vitest/coverage-istanbul

# Only when the app displays the optional results page
pnpm add @cross-code/vitest-ns-ui
```
...
Create `app/vitest-ns.ts`. Keeping `new Worker()` in application
...
Create `app/vitest-ns.worker.ts`:
...
} from '@cross-code/vitest-ns/runtime';
...
export default defineConfig({
  ...
  test: {
    coverage: {
      ...
      include: ['app/**/*.ts'],
      exclude: ['app/**/*.native.spec.ts', 'app/vitest-ns*.ts'],
