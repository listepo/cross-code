#!/usr/bin/env bash
# Builds NSCWasm3.xcframework (device arm64 + simulator arm64 + simulator
...
#   platforms/ios/NSCWasm3.xcframework        — shipped to the app
#   platforms/ios/NSCWasm3.xcframework.dSYMs/ — kept debug symbols (device+sim)
set -euo pipefail
cd "$(dirname "$0")/../platforms/ios/NSCWasm3"

OUT=../NSCWasm3.xcframework
DSYM_OUT=../NSCWasm3.xcframework.dSYMs
...
build_slice() { # $1=triple $2=sdk-name $3=output-name
  ...
  swift build --disable-sandbox -c release --product NSCWasm3 --triple "$triple" \
    -Xswiftc -sdk -Xswiftc "$sdk" \
    -Xcc -isysroot -Xcc "$sdk" \
    -Xswiftc -g -Xswiftc -Osize \
    -Xcc -g -Xcc -flto=thin \
    -Xlinker -dead_strip
  local dylib=".build/$triple/release/libNSCWasm3.dylib"

  # Assemble the framework bundle around the dylib.
  local fw="$BUILD/$out_name/NSCWasm3.framework"
  mkdir -p "$fw/Headers" "$fw/Modules"
  cp "$dylib" "$fw/NSCWasm3"
  cp Sources/NSCWasm3/include/*.h "$fw/Headers/"
  ...
  # only and works).
  cp Sources/NSCWasm3/include/module.modulemap "$fw/Modules/"
  ...
  install_name_tool -id "@rpath/NSCWasm3.framework/NSCWasm3" "$fw/NSCWasm3"
  /usr/libexec/PlistBuddy -c "Clear dict" \
    -c "Add :CFBundleIdentifier string org.nativescript.NSCWasm3" \
    -c "Add :CFBundleName string NSCWasm3" \
    -c "Add :CFBundleExecutable string NSCWasm3" \
  ...
  dsymutil "$fw/NSCWasm3" -o "$BUILD/$out_name/NSCWasm3.framework.dSYM" >/dev/null 2>&1 || true
}
...
strip -S -x "$BUILD/ios-arm64/NSCWasm3.framework/NSCWasm3"
strip -S -x "$BUILD/ios-arm64-simulator/NSCWasm3.framework/NSCWasm3"
strip -S -x "$BUILD/ios-x86_64-simulator/NSCWasm3.framework/NSCWasm3"
...
<dict>
	...
	<array>
		<dict>
			...
			<key>LibraryPath</key>
			<string>NSCWasm3.framework</string>
...
PLIST
ditto "$BUILD/ios-arm64/NSCWasm3.framework" "$OUT/ios-arm64/NSCWasm3.framework"
ditto "$BUILD/ios-arm64-simulator/NSCWasm3.framework" "$OUT/ios-arm64-simulator/NSCWasm3.framework"
ditto "$BUILD/ios-x86_64-simulator/NSCWasm3.framework" "$OUT/ios-x86_64-simulator/NSCWasm3.framework"

# ── 5. Keep the dSYMs produced by dsymutil (device + simulator). ─────────
for slice in ios-arm64 ios-arm64-simulator ios-x86_64-simulator; do
  ditto "$BUILD/$slice/NSCWasm3.framework.dSYM" "$DSYM_OUT/$slice/NSCWasm3.framework.dSYM"
done
