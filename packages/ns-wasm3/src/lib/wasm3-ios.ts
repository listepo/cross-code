...
function toNsArray(values: WireValue[]): NativeMutableArray {
  ...
  if (!NSMutableArray) throw new Wasm3Error('NSMutableArray not available');
...
class IosFunction implements NativeFunctionAdapter {
  constructor(private readonly fn: NSCWasm3FunctionRef) {}
  name(): string {
...
function makeIosHostCallback(cb: WireHostCallback): object {
  const Base = globalThis.NSCWasm3HostCallback;
...
class IosModule implements NativeModuleAdapter {
  constructor(
    private readonly module: NSCWasm3ModuleRef,
...
export class IosRuntime implements NativeRuntimeAdapter {
  private readonly runtime: NSCWasm3RuntimeRef;
  ...
  constructor(stackSizeInBytes: number) {
    const RuntimeClass = globalThis.NSCWasm3Runtime;
    if (!RuntimeClass) {
  ...
  readMemory(offset: number, length: number): Uint8Array {
    ...
    if (!buffer) throw new Wasm3Error('readMemory: interop.bufferFromData unavailable');
