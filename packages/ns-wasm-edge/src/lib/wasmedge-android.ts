...
import type {
  JavaArrayList,
  NativeWasmEdgeRuntimeProxy,
  NativeWasmEdgeModuleProxy,
  NativeWasmEdgeFunctionProxy,
  NativeScriptOrg,
  NsWasmEdgeNamespace,
} from './native-proxy.js';

function ns(): NsWasmEdgeNamespace | undefined {
  return (globalThis as unknown as NativeScriptOrg).org?.nativescript?.wasmedge;
}
...
function toJavaBytes(source: Uint8Array): JavaArrayList {
  try {
    return ns()?.NSCWasmEdgeRuntime.jsByteArrayToJava(
...
function fromJavaBytes(javaBytes: JavaArrayList): Uint8Array {
  try {
    return new Uint8Array(ns()!.NSCWasmEdgeRuntime.javaByteArrayToJs(javaBytes));
  } catch {
...
class AndroidFunction implements NativeFunctionAdapter {
  constructor(private readonly fn: NativeWasmEdgeFunctionProxy) {}
  name(): string {
  ...
  call(args: WireValue[]): WireValue[] {
    ...
    try {
      ...
      return javaArrayToJs(result).map((v) => {
        ...
        if (n === null) throw new WasmEdgeError(`${ctx}: null slot`);
        return n;
      });
    } catch (error) {
      rethrow(error, ctx);
...
function makeAndroidHostCallback(cb: WireHostCallback): unknown {
  const n = ns();
  if (!n?.NSCWasmEdgeHostCallback)
    throw new WasmEdgeError('NSCWasmEdgeHostCallback not available');
  // The Kotlin layer wraps the callback; pass through `unknown` to avoid narrowing.
  return new n.NSCWasmEdgeHostCallback(cb as (args: unknown[]) => unknown[]);
}

class AndroidModule implements NativeModuleAdapter {
  constructor(private readonly module: NativeWasmEdgeModuleProxy) {}
  name(): string {
  ...
  getGlobal(name: string): WireValue {
    try {
      ...
      if (value === null) throw new WasmEdgeError(`getGlobal ${name}: returned null`);
      return value;
    } catch (error) {
...
export class AndroidRuntime implements NativeRuntimeAdapter {
  private readonly runtime: NativeWasmEdgeRuntimeProxy;
  constructor(stackSizeInBytes: number) {
    const n = ns();
    if (!n?.NSCWasmEdgeRuntime)
      throw new WasmEdgeError('ns-wasm-edge native runtime not found on Android');
