...
import type { NativeScriptOrg } from './native-proxy.js';
...
export interface WasmEdgeImports {
  [module: string]: { [name: string]: { signature: string; fn: WasmEdgeHostFunction } };
}
export interface WasmEdgeRuntimeOptions {
  stackSizeInBytes?: number;
}

interface GlobalThisWithWasmEdge {
  org?: NativeScriptOrg['org'];
  NSCWasmEdgeRuntime?: unknown;
}

function createAdapter(stackSizeInBytes: number): NativeRuntimeAdapter {
  const g = globalThis as unknown as GlobalThisWithWasmEdge;
  if (typeof g.NSCWasmEdgeRuntime !== 'undefined' && g.NSCWasmEdgeRuntime !== null)
    return new IosRuntime(stackSizeInBytes);
  if (g.org?.nativescript?.wasmedge?.NSCWasmEdgeRuntime)
    return new AndroidRuntime(stackSizeInBytes);
  throw new WasmEdgeError(
    'ns-wasm-edge native runtime not found — is the plugin installed and the app rebuilt?',
  );
}
...
function wasmEdgeVersionNative(): string {
  ...
  if (typeof g.NSCWasmEdgeRuntime !== 'undefined' && g.NSCWasmEdgeRuntime !== null)
    return String(
      (g.NSCWasmEdgeRuntime as { wasmedgeVersion(): string }).wasmedgeVersion(),
    );
  if (g.org?.nativescript?.wasmedge?.NSCWasmEdgeRuntime)
    return String(
      (
        g.org.nativescript.wasmedge.NSCWasmEdgeRuntime as { wasmedgeVersion(): string }
      ).wasmedgeVersion(),
    );
...
export class WasmEdgeRuntime extends WasmRuntime {
  constructor(options?: WasmEdgeRuntimeOptions) {
    super(createAdapter(options?.stackSizeInBytes ?? 64 * 1024), {
      moduleCtor: WasmEdgeModule,
      functionCtor: WasmEdgeFunction,
    });
  }
  static version(): string {
    return wasmEdgeVersionNative();
  }
  override loadModule(source: WasmEdgeModuleSource, imports?: WasmEdgeImports): WasmEdgeModule {
    return super.loadModule(source, imports) as unknown as WasmEdgeModule;
  }
  override findFunction(name: string): WasmEdgeFunction {
    return super.findFunction(name) as unknown as WasmEdgeFunction;
  }
}
