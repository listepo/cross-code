// iOS (Swift/NSCWamr) platform adapter. The native classes are exposed on
// globalThis by the NativeScript iOS runtime.

import { WamrError, type WasmValueType, type WireValue } from './wire.js';
import type { WireHostCallback, NativeFunctionAdapter, NativeModuleAdapter, NativeRuntimeAdapter } from './wamr.js';
 ...
 * reference in rather than raising — so the wamr message is only reachable
...
class IosFunction implements NativeFunctionAdapter {
  ...
  call(args: WireValue[]): WireValue[] {
    ...
    if (result == null) throw new WamrError(`${context}: returned null`);
    return nsArrayToJs(result) as WireValue[];
  }
}

/**
 * Creates an NSCWamrHostCallback subclass whose overridden invoke() captures
...
class IosModule implements NativeModuleAdapter {
  ...
  getGlobal(name: string): WireValue {
    ...
    if (value == null) throw new WamrError(`${context}: global not found`);
...
export class IosRuntime implements NativeRuntimeAdapter {
  ...
  loadModuleFromBytes(bytes: Uint8Array): NativeModuleAdapter {
    ...
    if (!module) throw new WamrError('loadModule: returned null');
  ...
  loadModuleFromFile(path: string): NativeModuleAdapter {
    ...
    if (!module) throw new WamrError(`${context}: returned null`);
    return new IosModule(module, this.hostCallbacks);
  }
  ...
  findFunction(name: string): NativeFunctionAdapter {
    ...
    if (!fn) throw new WamrError(`${context}: function not found`);
