#!/bin/bash
# Build the Rust FFI library and generate Kotlin bindings for wry-kotlin.
#
...
#   ./build.sh android      # Cross-compile for Android ABIs
...
build_rust() {
    ...
    cp target/release/libwry_ffi.dylib "$host_dir/"
    echo "  -> Copied libwry_ffi.dylib to $host_dir/"
}
...
build_android() {
    ...
    for abi_pair in "${abis[@]}"; do
        ...
        cargo build -p wry-ffi --lib --release --target "$target" 2>&1 || {
            echo "  WARNING: Failed to build for $target — skipping"
            continue
        }

        mkdir -p "$jni_dir"
        cp "target/$target/release/libwry_ffi.so" "$jni_dir/libwry_ffi.so"
        echo "  -> Copied libwry_ffi.so to $jni_dir/"
    done
}
