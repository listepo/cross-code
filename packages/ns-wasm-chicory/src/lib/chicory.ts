...
export interface ChicoryImports {
  [module: string]: { [name: string]: { signature: string; fn: ChicoryHostFunction } };
}
export interface ChicoryRuntimeOptions {
  stackSizeInBytes?: number;
}

interface GlobalThisWithChicory {
  org?: NativeScriptOrg['org'];
  NSCChicoryRuntime?: unknown;
}

function createAdapter(stackSizeInBytes: number): NativeRuntimeAdapter {
  const g = globalThis as unknown as GlobalThisWithChicory;
  if (g.org?.nativescript?.chicory?.NSCChicoryRuntime)
    return new AndroidRuntime(stackSizeInBytes);
  if (typeof g.NSCChicoryRuntime !== 'undefined' && g.NSCChicoryRuntime !== null)
...
function chicoryVersionNative(): string {
  ...
  if (g.org?.nativescript?.chicory?.NSCChicoryRuntime)
    return String(
      (
        g.org.nativescript.chicory.NSCChicoryRuntime as { chicoryVersion(): string }
      ).chicoryVersion(),
    );
  if (typeof g.NSCChicoryRuntime !== 'undefined' && g.NSCChicoryRuntime !== null)
    return String(
      (g.NSCChicoryRuntime as { chicoryVersion(): string }).chicoryVersion(),
    );
  throw new ChicoryError('ns-wasm-chicory native runtime not found');
}
...
export class ChicoryRuntime extends WasmRuntime {
  constructor(options?: ChicoryRuntimeOptions) {
    super(createAdapter(options?.stackSizeInBytes ?? 64 * 1024), {
      moduleCtor: ChicoryModule,
      functionCtor: ChicoryFunction,
    });
  }
  static version(): string {
    return chicoryVersionNative();
  }
  override loadModule(source: ChicoryModuleSource, imports?: ChicoryImports): ChicoryModule {
    return super.loadModule(source, imports) as unknown as ChicoryModule;
  }
  override findFunction(name: string): ChicoryFunction {
    return super.findFunction(name) as unknown as ChicoryFunction;
  }
}
